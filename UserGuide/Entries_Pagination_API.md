...
> Tài liệu mô tả 4 API tra cứu lô nhập/xuất kho: `GetReceiptEntriesByDate`, `GetIssueEntriesByDate` (tìm theo khoảng ngày) và `GetReceiptEntriesByLotNumber`, `GetIssueEntriesByLotNumber` (tìm gần đúng theo **mã lô hàng** và/hoặc **tên sản phẩm**) — cả 4 đều dùng chung cơ chế **phân trang tại Backend** (`pageNumber`, `pageSize`), filter theo **tên kho** (`warehouseName`), và **sắp xếp theo tiến độ**, thay cho việc trước đây FE phải tải toàn bộ danh sách rồi tự phân trang ở client.
>
...
|---|---|---|---|
| GET | `/WarehouseAPI/InventoryReceiptEntry/GetReceiptEntriesByDate` | `Manager`, `Admin` | Khoảng ngày (`fromDate`/`toDate`) |
| GET | `/WarehouseAPI/InventoryIssueEntry/GetIssueEntriesByDate` | `Manager`, `Admin` | Khoảng ngày (`fromDate`/`toDate`) |
...
## 3. Thứ tự sắp xếp (Sort Order) — theo tiến độ

Kết quả trả về (cả khi phân trang lẫn khi trả full list) luôn được **Backend sắp xếp sẵn theo tiến độ lô hàng** (`LotStatus` của `receiptLot`/`issueLot`), FE **không cần** tự sort lại:
...
⚠️ Việc sort theo tiến độ được áp dụng **trước** khi cắt trang (`Skip`/`Take`) — nghĩa là trang 1 sẽ luôn ưu tiên hiển thị các lô "Đang thực hiện"/"Chờ xử lý" trước, đúng với mục đích UI muốn người dùng thấy ngay các lô cần xử lý gấp lên đầu danh sách.

## 4. Query Parameters

### 4.1. `GetReceiptEntriesByDate` / `GetIssueEntriesByDate`
...
- Không truyền cả `lotNumber` lẫn `materialName` → API trả về toàn bộ (chỉ áp dụng `warehouseName`/phân trang nếu có) — tương đương gọi `GetReceiptEntriesByDate`/`GetIssueEntriesByDate` không kèm `fromDate`/`toDate`.
- Lưu ý: 2 API này **không có** `fromDate`/`toDate`. Nếu cần tìm theo ngày **và** theo mã lô/tên sản phẩm cùng lúc cho 1 màn hình, báo lại để gộp `lotNumber`/`materialName` như filter phụ vào `GetReceiptEntriesByDate`/`GetIssueEntriesByDate` thay vì gọi 2 API riêng.
...
### 5.1. Cấu trúc item — `GetReceiptEntriesByDate` (`InventoryReceiptEntryDTO`)
...
```ts
{
  ...
  receiptLot: {
    ...
    receiptLotStatus: string   // enum dạng string, VD "Pending" | "Done"
    inventoryReceiptEntryId: string
    materialId: string
    materialName: string
    storageLevel: string
...
### 5.2. Cấu trúc item — `GetIssueEntriesByDate` (`InventoryIssueEntryDTO`)
...
```
GET /WarehouseAPI/InventoryReceiptEntry/GetReceiptEntriesByDate?fromDate=2026-08-01&toDate=2026-08-22&warehouseName=Kho%20Nguy%C3%AAn%20v%E1%BA%ADt%20li%E1%BB%87u&pageNumber=1&pageSize=20
```

**Không phân trang, chỉ lọc theo ngày (hành vi tương thích ngược):**
```
GET /WarehouseAPI/InventoryIssueEntry/GetIssueEntriesByDate?fromDate=2026-08-01&toDate=2026-08-22
```
...
5. Vì kết quả đã được sắp theo tiến độ sẵn (mục 3), UI có thể hiển thị badge/màu tương ứng theo nhóm trạng thái (VD: xanh dương = Đang thực hiện, vàng = Chờ xử lý, xanh lá = Hoàn thành, cam = Tạm hoãn, đỏ = Bị chặn, xám = Đã huỷ) mà **không cần** gọi thêm API hay sort lại ở client.
